...
extern pthread_mutex_t input_lock, output_lock;
extern queue <double> input_q;
extern queue <double> output_q;
...
//			cout<<big_pmap[n].x-st.x<<' '<<big_pmap[n].y-st.y<<endl;
			LOCK(output_lock);
			output_q.push(big_pmap[n].x-st.x);
			output_q.push(big_pmap[n].y-st.y);
			UNLOCK(output_lock);
		}
	}
...
//		cout<<dir1.x<<' '<<dir1.y<<' '<<PI/180.0<<endl;
		LOCK(output_lock);
		output_q.push(dir1.x);
		output_q.push(dir1.y);
		output_q.push(PI/180.0);
		UNLOCK(output_lock);
